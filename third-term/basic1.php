...
if(isset($_POST['submit'])){
    ...
    if (empty($sub_name)){
      ...
      $queryc=mysqli_query($con3,"insert into basic1_subject_list (subject_id, subject_name) value ('$sub_id','$sub_name')");
...
if(isset($_POST['delete'])){
  ...
  $queryb = mysqli_query($con2,"delete from basic1_subject_list where subject_id = '$sub_id'");
  $queryc = mysqli_query($con3,"delete from basic1_subject_list where subject_id = '$sub_id'");

  $new = 0;
  $query2 = mysqli_query($con,"update basic1_".$sub_id." set `first test` = '$new', `second test` ='$new', `exam` = '$new', `Total` = '$new', `grade` = '', `position` = '' ");
  $query2b = mysqli_query($con2,"update basic1_".$sub_id." set `first test` = '$new', `second test` ='$new', `exam` = '$new', `Total` = '$new', `grade` = '', `position` = '' ");
  $query2c = mysqli_query($con3,"update basic1_".$sub_id." set `first test` = '$new', `second test` ='$new', `exam` = '$new', `Total` = '$new', `grade` = '', `position` = '' ");
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper" >
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">List Of Subjects for Basic 1</h1>
...
<form role="form" name="assign"  method="POST" >
    <div class="form-group" > 
        ...
        <select name="sub_id"class="form-control me"  style = "width:200px; margin:4px;" required>
            ...
            <?php 
             $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject1'");
             if (mysqli_num_rows($query) == 0) {
            ?>     
            <option value="subject1">subject1</option>
             <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject2'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
            <option value="subject2">subject2</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject3'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
            <option value="subject3">subject3</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject4'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
            <option value="subject4">subject4</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject5'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
            <option value="subject5">subject5</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject6'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
            <option value="subject6">subject6</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject7'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
            <option value="subject7">subject7</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject8'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
            <option value="subject8">subject8</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject9'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
            <option value="subject9">subject9</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject10'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
            <option value="subject10">subject10</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject11'");
              if (mysqli_num_rows($query) == 0) {     
             ?>                                            
            <option value="subject11">subject11</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject12'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
            <option value="subject12">subject12</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject13'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
            <option value="subject13">subject13</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject14'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
            <option value="subject14">subject14</option>
            <?php } 
              $query= mysqli_query($con3,"select * from basic1_subject_list where subject_id = 'subject15'");
              if (mysqli_num_rows($query) == 0) {     
             ?>
...
<hr>
    ...
    <table class="table table-bordered">
      ...
      <?php
          ...
          $query= mysqli_query($con3,"select * from basic1_subject_list order by length(subject_id), subject_id");
          if (mysqli_num_rows($query) > 0) {
